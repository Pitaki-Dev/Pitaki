...
**仓库已可运行**（`pnpm tauri:dev` 能启动窗口），但**尚无实际阅读功能** —— 界面外壳已搭好，
三个页面中只有「设置」具备真实功能，**阅读器尚未接入引擎**。下面列出的特性是**目标特性**，
请勿据此判断本项目已可用。
...
| 工程骨架搭建 | ✅ 已完成（Vite 8 + React 19 + TS 7 + Tauri v2，CI 全绿） |
| 界面外壳（响应式 + 触摸） | ✅ 已完成（外壳可运行，页面暂为占位）→ [docs/UI.md](docs/UI.md) |
| **引擎接入与阅读功能** | 📋 **未开始 —— 下一步** → [docs/ROADMAP.md](docs/ROADMAP.md) |
...
| 特性 | 说明 |
|---|---|
| 多格式 | EPUB、MOBI、KF8（AZW3）、FB2、CBZ、**TXT** |
...
> 以下命令**当前可用**。首次克隆后必须先跑 `pnpm run build:vendor`。
